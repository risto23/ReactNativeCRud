...
export default class HasilActivity extends React.Component {
  ...
  componentDidMount(){
      return fetch('http://10.100.100.205/react-native-web/view_user.php')
        .then((response) => response.json())
        .then((responseJson) =>
        {
  ...
  Action_Click = (id,nama,email,telp,tgl_lahir,jk,pekerjaan) => {
  ...
  _renderItem = ({ item }) => (
            ...
            <View style={styles.tulisan_background}>
               {/* <Text style={styles.tulisan1}> {item.id} </Text> */}
                <Text style={styles.tulisan1}> {item.nama} </Text>
                {/* <Text style={styles.tulisan2}>{item.firstname}</Text> */}
                {/* <Text style={styles.tulisan3}>{item.lastname}</Text> */}
                ...
                {/* <TouchableOpacity onPress={this.Action_Click.bind(this,
                    ...
                    <FontAwesomeIcon style={styles.btnText} icon={faTrash} />
                </TouchableOpacity> */}
